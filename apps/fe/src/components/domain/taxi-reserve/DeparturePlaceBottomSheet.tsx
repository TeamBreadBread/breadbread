...
const DEFAULT_RECENTS = ["검색어 1", "검색어 2", "검색어 3"];
...
function loadRecents(): string[] {
  try {
    ...
    if (!Array.isArray(parsed)) return [...DEFAULT_RECENTS];
    const list = parsed.filter((x): x is string => typeof x === "string" && x.trim().length > 0);
    return list.length > 0 ? list : [...DEFAULT_RECENTS];
  } catch {
