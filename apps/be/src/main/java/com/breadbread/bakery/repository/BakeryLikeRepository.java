...
public interface BakeryLikeRepository extends JpaRepository<BakeryLike, Long> {
    ...
    @Query("SELECT bl.bakery.id, COUNT(bl) FROM BakeryLike bl WHERE bl.bakery.id IN :bakeryIds GROUP BY bl.bakery.id")
    List<Object[]> countByBakeryIdIn(@Param("bakeryIds") List<Long> bakeryIds);

	// 유저가 좋아요한 bakeryId 목록 조회
	@Query("SELECT bl.bakery.id FROM BakeryLike bl WHERE bl.bakery.id IN :bakeryIds AND bl.user.id = :userId")
	List<Long> findLikedBakeryIdsByUserId(@Param("bakeryIds") List<Long> bakeryIds, @Param("userId") Long userId);
}
