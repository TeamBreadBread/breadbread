...
public class AiWebhookClient {
	...
	public AiCourseWebhookResponse requestCourse(String jobId, AiCourseWebhookRequest request) {
		log.info("[AI 웹훅] 요청 시작: jobId={}, bakeryCount={}",
			jobId, request.getBakeries() != null ? request.getBakeries().size() : 0);
			...
			rawBody = client.post()
				...
				.onStatus(HttpStatusCode::isError, res ->
					res.bodyToMono(String.class)
						.flatMap(body -> {
							log.error("[AI 웹훅] HTTP 오류 응답: jobId={}, status={}, body={}", jobId, res.statusCode(), body);
				...
				.block();
			log.info("[AI 웹훅] 응답 수신 완료: jobId={}, elapsed={}ms", jobId, System.currentTimeMillis() - start);
		} catch (CustomException e) {
			throw e;
		} catch (Exception e) {
			log.error("[AI 웹훅] 웹훅 호출 실패: jobId={}, elapsed={}ms", jobId, System.currentTimeMillis() - start, e);
		...
		try {
			AiCourseWebhookResponse response = objectMapper.readValue(rawBody, AiCourseWebhookResponse.class);
			log.debug("[AI 웹훅] 응답 파싱 완료: jobId={}, name={}, bakeryCount={}",
				jobId, response.getName(),
