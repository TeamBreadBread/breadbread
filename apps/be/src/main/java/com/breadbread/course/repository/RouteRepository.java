...
public interface RouteRepository extends JpaRepository<Route, Long> {
    ...
    @Query(
            "SELECT bl.course.id, COUNT(bl) FROM Route bl WHERE bl.course.id IN :courseIds GROUP BY bl.course.id")
    List<Object[]> countByCourseIdIn(@Param("courseIds") List<Long> courseIds);

    // 유저가 루트로 저장한 courseId 목록 조회
    @Query(
            "SELECT bl.course.id FROM Route bl WHERE bl.course.id IN :courseIds AND bl.user.id = :userId")
