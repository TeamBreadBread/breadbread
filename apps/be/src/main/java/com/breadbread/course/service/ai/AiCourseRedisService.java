...
public class AiCourseRedisService {
    ...
    public Optional<AiJobStatusResponse> findByJobId(String jobId, Long requesterId) {
        return findCacheByJobId(jobId).map(cache -> {
            if (cache.getUserId() == null || !cache.getUserId().equals(requesterId)) {
                throw new CustomException(ErrorCode.FORBIDDEN);
            }
            return new AiJobStatusResponse(cache.getStatus(), cache.getCourseId(), cache.getErrorMessage());
        });
    }
    ...
    private void save(String jobId, AiJobCache value) {
        try {
            stringRedisTemplate.opsForValue().set(JOB_PREFIX + jobId, objectMapper.writeValueAsString(value), Duration.ofHours(aiProperties.getJobTtlHours()));
        } catch (JsonProcessingException e) {
