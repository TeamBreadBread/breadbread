...
public class AiCourseAsyncService {
	...
	public CompletableFuture<Void> processAiCourse(String jobId, Long userId, AiCourseRequest request) {
		try {
			...
			AiCourseWebhookRequest webhookRequest = transactionTemplate.execute(status -> {
				...
				List<BakeryAiResponse> bakeryAiResponses = bakeries.stream()
			...
			if (response == null || response.getName() == null
				|| response.getEstimatedCost() == null || response.getEstimatedTime() == null
				|| response.getBakeries() == null || response.getBakeries().isEmpty()) {
			...
			Long courseId = transactionTemplate.execute(status -> {
				...
				List<Long> recommendedIds = response.getBakeries().stream()
					.map(RecommendedBakeryResponse::getId).toList();
				Map<Long, Bakery> bakeryMap = bakeryRepository.findAllById(recommendedIds)
					.stream().collect(Collectors.toMap(Bakery::getId, b -> b));
				...
				AiCourseInfo aiCourseInfo = AiCourseInfo.builder()
				...
				Course course = Course.createAi(response.getName(), user, userPreference,
					aiCourseInfo, new HashSet<>(request.getBreadTypes()));
				course.updateAiResult(response.getEstimatedCost(), response.getEstimatedTime(),
					response.getTheme(), response.getSummary());
				...
				response.getBakeries().stream()
					...
					.forEach(item -> {
						CourseBakery cb = CourseBakery.builder()
