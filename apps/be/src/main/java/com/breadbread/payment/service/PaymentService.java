...
public class PaymentService {
	...
	@Transactional
	public void handleWebhook(String rawBody, HttpServletRequest request) throws WebhookVerificationException {
		String secret = properties.getWebhookSecret();
		if (secret == null || secret.isBlank()) {
			log.warn("[포트원 웹훅] webhook secret 미설정");
			throw new ResponseStatusException(
				HttpStatus.BAD_REQUEST,
				"PortOne webhook secret is not configured"
			);
		}
		...
		String paymentId = (data != null && data.hasNonNull("paymentId"))
	...
	private void syncCancelledFromPortOneByWebhook(String paymentId) {
		...
		try {
			...
			if (reservation.getStatus() == com.breadbread.reservation.entity.ReservationStatus.CONFIRMED) {
				reservation.cancel();
			}
	...
	private boolean amountMatches(Payment payment, PortOnePaymentResponse response) {
		return response.getAmount() != null && response.getAmount().getTotal() == payment.getFinalAmount();
	}

	private void confirmPaidPayment(Payment payment, PortOnePaymentResponse response) {
		LocalDateTime paidAt = response.getStatusChangedAt() != null
