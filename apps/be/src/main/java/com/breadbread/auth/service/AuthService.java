...
public class AuthService {
    ...
    public void signup(SignupRequest signupRequest) {
        ...
        phoneVerificationRedisService.deleteByVerificationToken(
    ...
    public TokenResponse login(LoginRequest loginRequest) {
        ...
        authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(
                        user.getId().toString(), loginRequest.getPassword()));
    ...
    public FindIdResponse findId(FindIdRequest findIdRequest) {
        ...
        User user =
                userRepository
                        .findByPhone(findIdRequest.getPhone())
                        .orElseThrow(() -> new CustomException(ErrorCode.USER_NOT_FOUND));
        if (!user.getName().equals(findIdRequest.getName())
                || !user.getPhone().equals(verification.getPhone())) {
    ...
    public void findPassword(FindPwRequest findPwRequest) {
        PhoneVerificationCache verification =
                validateVerificationToken(
                        findPwRequest.getVerificationToken(), VerificationPurpose.FIND_PW);
        User user =
                userRepository
                        .findByLoginIdIgnoreCase(normalizeLoginId(findPwRequest.getLoginId()))
                        .orElseThrow(() -> new CustomException(ErrorCode.USER_NOT_FOUND));
        if (!user.getName().equals(findPwRequest.getName())
                || !user.getPhone().equals(verification.getPhone())) {
            throw new CustomException(ErrorCode.USER_INFO_MISMATCH);
        }
    }
    ...
    public void resetPassword(ResetPwRequest resetPwRequest) {
        ...
        User user =
                userRepository
                        .findByPhone(verification.getPhone())
                        .orElseThrow(() -> new CustomException(ErrorCode.USER_NOT_FOUND));
    ...
    public void sendVerificationCode(SendPhoneRequest sendPhoneRequest) {
        phoneVerificationRedisService.deleteByPhoneAndPurpose(
                sendPhoneRequest.getPhone(), sendPhoneRequest.getPurpose());
        ...
        log.info(
                "인증번호 발송 phone={} purpose={}",
                maskPhone(sendPhoneRequest.getPhone()),
                sendPhoneRequest.getPurpose());
    ...
    public VerifyPhoneResponse verifyCode(VerifyPhoneRequest verifyPhoneRequest) {
        PhoneVerificationCache verification =
                phoneVerificationRedisService
                        .findByPhoneAndPurpose(
                                verifyPhoneRequest.getPhone(), verifyPhoneRequest.getPurpose())
        ...
        log.info(
                "휴대전화 인증 완료 phone={} purpose={}",
                maskPhone(verifyPhoneRequest.getPhone()),
                verifyPhoneRequest.getPurpose());
    ...
    private PhoneVerificationCache validateVerificationToken(
            String token, VerificationPurpose purpose) {
        PhoneVerificationCache verification =
                phoneVerificationRedisService
